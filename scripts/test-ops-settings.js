...
(function () {
  ...
  var notifyDef = T.OPS_PREF_FIELDS.filter(function (f) { return f.k === 'notify.channel'; })[0].def;
  var d = T.bwnCfgPartial(ALL, rawFrom(DEFAULTS, { 'notify.channel': notifyDef }), DEFAULTS);
  A.ok('select at its default (' + notifyDef + ') writes no group', !('notify' in d.partial));
