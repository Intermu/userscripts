...
//
//   - Source Job # = the RFQ ID suffixed " (FM-AMZ)"   e.g. 4100777 -> "4100777 (FM-AMZ)"
...
// no DOM is needed.
//
// FIXTURES ARE SANITIZED. The body below reproduces the SHAPE of a real Fairmarkit RFQ email (the
// anchors each regex depends on, in the order the parser expects) with every identifier replaced by
// a synthetic one: buyer name, RFQ ID, site code, street address and zip are invented. Only the
// city/state survive, because the Location matcher scores on them and they identify a commercial
// facility rather than a person. Do NOT paste a real client email in here - this repo is
// anonymously readable, and the assert helper prints `got`/`want` into public CI logs on failure.
//
...
var SUBJECT = 'Amazon.com, Inc. - Request for Quote #4100777';
// A realistic RFQ body in the Fairmarkit shape, grounded on the real #4100777 email: site DEX9,
// shipping to 410 Example Parkway, Fenton, MO. Ordered so each parse terminator is present
// (Notes -> "View more", Shipping -> "Preferred delivery date:").
var BODY = [
  'Amazon.com, Inc. - Request for Quote',
  'Invitation to Quote DEX9 You have been invited to quote on the following request.',
  'Internal part # QTY 1. Replace damaged dock bumper 2.00 Notes to supplier: Access via the receiving dock. View more',
  'Buyer: Dana (Danaq) Quinn Close date: Aug 14, 2026 at 10:00 AM (GMT-08:00) Pacific Time - Los Angeles RFQ ID: 4100777',
  'Shipping address: 410 Example Parkway, Fenton, MO, 63099, US Preferred delivery date: Aug 20, 2026',
...
console.log('# real RFQ #4100777 (site DEX9, Fenton MO)');
...
A.ok('  RFQ ID captured = 4100777', ax.rfqId === '4100777', 'got ' + JSON.stringify(ax.rfqId));
A.ok('  Site code = DEX9', ax._siteCode === 'DEX9', 'got ' + JSON.stringify(ax._siteCode));
A.ok('  Location search term = DEX9', ax.location === 'DEX9', 'got ' + JSON.stringify(ax.location));
...
A.ok('  Address street # = 410', ax._addr && ax._addr.streetNum === '410', 'got ' + JSON.stringify(ax._addr && ax._addr.streetNum));
A.ok('  Preferred delivery date -> _dueBy', /Aug\s+20,?\s*2026/.test(ax._dueBy || ''), 'got ' + JSON.stringify(ax._dueBy));
A.ok('  refs note carries "RFQ #4100777"', (ax._note || '').indexOf('RFQ #4100777') >= 0, 'got ' + JSON.stringify(ax._note));
...
A.eq('  Source Job # = "4100777 (FM-AMZ)"', m.sourceJob, '4100777 (FM-AMZ)');
...
A.ok('subject-only path (buyer named, non-Fairmarkit sender) still detects',
  api.isAmazon('procurement@example.com', 'Amazon.com, Inc. - Request for Quote #4100777', '') === true);
