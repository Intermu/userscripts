...
(function () {
  ...
  var VER = '0.2.0';
  console.info('[BWN KANBAN] v' + VER + ' - board view over the captured PagedWorkOrders query; drag between status lanes writes via captured PatchWorkOrder');
  ...
  function injectCSS() {
    ...
    st.textContent = [
      ...
      '#bwn-kanban .kb-card{background:#fff;border:1px solid #e2e8e5;border-radius:8px;padding:8px 10px;cursor:grab;box-shadow:0 1px 2px rgba(0,0,0,.04);}',
  ...
  function buildCard(r) {
    ...
    // When the lanes are NOT status, the status still has to be readable on the card.
    if (group !== 'status' && r.statusName) chips.appendChild(el('span', 'kb-chip', r.statusName));
    var dne = money(r.doNotExceed);
    if (dne) chips.appendChild(el('span', 'kb-chip kb-dne', dne));
    var onsite = fmtDate(r.nextOnsiteDate);
    if (onsite) chips.appendChild(el('span', 'kb-chip', 'onsite ' + onsite));
    if (r.timeInStatus != null) chips.appendChild(el('span', 'kb-chip', Math.round(r.timeInStatus) + 'h in status'));
  ...
  function writeStatus(row, targetLaneKey) {
    ...
    var ok = confirm('Change W-' + row.number + ' status?\n\n' +
      ...
      'time-in-status clock (currently ' + (row.timeInStatus != null ? Math.round(row.timeInStatus) + 'h' : 'unknown') + '), which cannot be undone by\n' +
  ...
  function buildLane(name, items) {
    var lane = el('div', 'kb-lane');
  ...
  function boardNode() {
    ...
    order.forEach(function (k) {
      byKey[k].sort(function (a, b) { return (Number(b.numberOfDays) || 0) - (Number(a.numberOfDays) || 0); });
      lanes.appendChild(buildLane(k, byKey[k]));
    });
    if (!order.length && !scanState.running) lanes.appendChild(el('div', 'kb-note', 'No work orders in the current filter.'));
