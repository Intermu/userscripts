...
(function () {
  ...
  function injectCSS() {
    ...
    st.textContent = [
      ...
      // `flex:0 0 auto` on the card itself is REQUIRED, not optional: the card is a flex item of
      // the scrolling `.kb-cards` column, so with the default flex-shrink:1 a busy lane (30+
      // cards) shrinks every card to fit the lane's max-height and they collapse to ~26px - just
      // the header - while a sparse lane (12 cards) barely shrinks and looks fine. 0.7.2 shipped
      // without this line and did exactly that on the live board. Pinning shrink:0 makes the LANE
      // scroll instead of the cards collapsing. (`> *` below stops the card's own CHILDREN from
      // shrinking, so overflow becomes a card scroll rather than a silent clip - a different job.)
      '#bwn-kanban .kb-card{background:#fff;border:1px solid #e2e8e5;border-radius:12px;padding:12px 15px;cursor:grab;box-shadow:0 1px 2px rgba(0,0,0,.04);border-left:4px solid transparent;display:flex;flex-direction:column;flex:0 0 auto;gap:2px;max-height:250px;overflow-y:auto;overflow-x:hidden;}',
      ...
      // Header scrolls WITH the card (0.7.3 pinned it sticky, but the eyebrow/location scrolled
      // underneath and ghosted through the pinned WO# - it read as a weird floating double-image).
      '#bwn-kanban .kb-top{display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:2px;}',
  ...
  // 0.9.0: honour Core's per-user preference (Ops Suite panel > Preferences > Default WO view,
  // stored at bwn:config.view.defaultWO by bwn-suite-core 1.82.0). 'board' forces the board on
  // every list-page load; absent or 'list' keeps the remembered toggle, which is exactly the
  // pre-0.9.0 behaviour. The toggle button still writes LS_VIEW, so a forced board can be
  // flipped to the table for the rest of the session. Applies at load, not live - a panel save
  // takes effect on the next list page. Pure: sliced by scripts/test-kanban-card.js.
  function initialView(toggle, cfgRaw) {
    try { var c = JSON.parse(cfgRaw || 'null'); if (c && c.view && c.view.defaultWO === 'board') return 'board'; } catch (e) { /* malformed blob - fall through */ }
    return toggle === 'board' ? 'board' : 'list';
  }
  var view = initialView(storeGet(LS_VIEW, 'list'), storeGet('bwn:config', null));
  ...
  // Routes the drag status write through bwnGqlOp (the paste-identical BWN-OPS-WRAP below,
  // SHA-gated to Core) so it gets a correlation id + the shared bwn:audit entry + the
  // fail-closed high-risk confirm gate + centralized success:false rejection - the same spine
  // dispatch/proposal-actions' patchWorkOrder rides. Kanban's transport is the CAPTURED-session
  // fetch (the Auth0 bearer lives in lastReq.headers; a bare same-origin POST is rejected 'No
  // authentication method provided'), so bwnGql POSTs over lastReq.headers. Only patchWorkOrder
  // is routed; the board's reads stay on the scan. writeStatus confirms in its own dialog.
  var bwnGql = function (query, variables) {
    return fetch('/api/graphql', {
      method: 'POST',
      credentials: 'include',
      headers: lastReq.headers,
      body: JSON.stringify({ operationName: PATCH_OP, query: query, variables: variables || {} })
  ...
  // Ack state is LIVE, and it has to be re-read at RENDER time, not at pull time. 0.6.1 read it
  // once inside pullRows and cached the flag into heatMap, which looks right and is not: a
  // snooze made from List Heat's audit panel changes no rows, so no pull happens, so the board
  // kept rendering the stale flag. MEASURED at the live gate - the ack store said snoozed, the
  // card still drew a red edge.
  //
  // Refreshed once per render rather than once per card: heatOf is called three times per row
  // (card, lane count, sort), and a localStorage read + JSON parse per call would be ~650 reads
  // on a 219-row board. If that ever shows up in a profile, the fix is a bulk accessor in Core
  // rather than moving this back to pull time.
  function refreshAcks() {
    var keys = Object.keys(heatMap);
    for (var i = 0; i < keys.length; i++) heatMap[keys[i]].acked = liveAcked(heatMap[keys[i]]);
  }
