...
(function () {
  ...
  bwnBoot('listHeat', BWN_MODULES.listHeat, function () {
    ...
    // ---- Row snapshot for out-of-module renderers (v1.75.0, kanban fold) -------------
    // Built ONCE per clean API scan and cached; every reader gets the same frozen object.
    // Rebuilding per read was rejected because the kanban asks from five call sites, and
    // handing out the live store was rejected outright - a mutable handle on the authority's
    // own state is the two-writers-drift shape this module already carries scars from.
    //
    // Contract, deliberately narrow:
    //   __bwnHeatRows()  -> { ok:true, ts, sig, rows:[ {raw, id, wo, sev, reasons, kinds,
    //                         acked, warn, bad, status, hrs} ] }        (frozen, deeply)
    ...
    // consumer never computes a verdict - the rule the kanban card was already built on.
    //
    // `acked` is included but is a SNAPSHOT-TIME value and goes stale the moment anyone
    // snoozes from the audit panel. A live consumer should call __bwnHeatAck instead; this
    // field exists so a reader that does not care is not forced to.
    function heatRowsBuild() {
      ...
      // Object.freeze is SHALLOW, which is why reasons and kinds are frozen individually
      // above and the array itself below. `raw` is deliberately NOT frozen: it is the wire's
      // own nested object graph, freezing it deeply would cost a walk per scan, and no
      // consumer has any reason to write to it.
      // No filter signature on the payload: `heatAutoSig` lives in a different scope from this
      // builder, no consumer reads it, and reaching across for a field nobody wants is how a
      // cheap nicety becomes a ReferenceError at scan time. Caught by test-heat-api-scan, which
      // runs finishApi for real - the kanban harness alone would not have seen it, because it
      // calls heatRowsBuild directly.
    ...
    // are distinguishable ON PURPOSE: 'no capture yet' means reload the list, 'scan degraded'
    // means press Scan All, 'never scanned' means wait or force one. A single null would make
    // all three read the same and push the consumer into guessing.
    // Rebuild the cache and announce it. Fire-and-forget: the event carries NO payload, so a
    // listener that misses it (registered late, or busy) loses nothing - it pulls on its next
    // render. Push is for timing only; __bwnHeatRows is the only way to get data.
    function heatRowsPublish() {
      heatRowsCache = heatRowsBuild();
      if (!heatRowsCache) return 0;
      try { document.dispatchEvent(new CustomEvent('bwn:evt', { detail: { id: 'bwn:heat:rows' } })); } catch (e) { }
      return heatRowsCache.rows.length;
    }
    ...
    // Kept SEPARATE from heatStore on purpose (2026-08-09): heatStore entries are consumed by
    ...
    // three consumers - one of which posts to the SWA. This map has exactly one consumer,
    // heatRowsBuild below.
    //
    ...
    var heatRaw = null;
    // Frozen row snapshot for out-of-module consumers, rebuilt once per CLEAN api scan and
    // cached - never rebuilt per read, because the board asks from five call sites.
    ...
    function heatAutoScan(vars, force) {
      ...
      // THE GUARD THIS FORCE FLAG EXISTS TO BYPASS (added v1.75.0). A caller verifying a WRITE
      // it just made has the same filters it had a second ago, so `sig` is unchanged and the
      // TTL has not expired - meaning this early return fires, no scan runs, and the caller
      // reads back the PRE-WRITE store and calls the write verified. Measured against the
      // kanban board's drag path, which is the suite's only Umbrava write mutation.
      // A forced scan must therefore skip this line and only this line.
      if (!force && sig && sig === heatAutoSig && heatStore && (Date.now() - heatAutoTs) < HEAT_AUTO_TTL) return;
      heatAutoSig = sig;
      heatAutoTs = Date.now();
    ...
    //
    // An in-flight scan is AWAITED, never duplicated: two concurrent replays writing one
    // store is the drift shape, and the running scan is already going to produce the fresh
    // read the caller wants. Resolves false when no scan is possible at all (no capture, not
    // a list page, auto-heat off) so the caller can say so rather than hang.
    var heatForceWait = null;
    function heatForceScan() {
      if (heatScanning || heatReplaying) {
        if (heatForceWait) return heatForceWait;
        // A scan started by someone else, with no promise to join: poll the flags rather
        // than duplicate it. Bounded, because a wedged scan must not hang the caller.
    ...
    function apiScanAll(btn) {
      ...
      function finishApi(clean, note) {
        ...
        // The row snapshot is rebuilt from scratch on every finish, clean or not: a dirty
        // finish must leave NO cache behind, or __bwnHeatRows would keep serving the previous
        // scan's board as if it were this one's.
        heatRowsCache = null;
        ...
        if (clean && heatStore) { heatPublishVerdicts(heatStore); heatQueueDataset(heatStore); heatRowsPublish(); }
    ...
    function scanAll(btn) {
      ...
      heatStore = {};
      // The scroll sweep reads the DOM, so there are no raw API rows behind it. Null rather
      // than left over: a consumer joining heatStore to a PREVIOUS scan's raw rows would render
      // one scan's severity against another scan's row data.
    ...
    // ---- Row surface for out-of-module renderers (v1.75.0) ---------------------------
    // Three functions, no state. Consumers are in OTHER Tampermonkey scripts, which is only
    // possible because every participant is `@grant none` and therefore shares the page's
    // window - a GM_* grant on either side puts it in a sandbox where these are invisible.
    //
    // Pull, never push: the event below carries no payload, so a consumer that loads late,
    // misses it, or re-renders for its own reasons always gets the current answer by asking.
    window.__bwnHeatRows = function () {
      ...
      // the publish. Build once on demand rather than reporting empty.
    ...
    // on every render would put the duplicate full-board scan straight back.
    window.__bwnHeatScan = function (opts) {
      if (opts && opts.force) return heatForceScan();
      // Unforced: honour the TTL exactly as the auto path does, and still hand back a promise.
    ...
    // itself, which is what this module does for its own rows (see addSnooze). A consumer
    // that renders `acked` off the snapshot will disagree with the list the moment anyone
    // snoozes, which is exactly the divergence the shared verdict exists to prevent.
